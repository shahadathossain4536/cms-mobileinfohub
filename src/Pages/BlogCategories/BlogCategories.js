...
const BlogCategories = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (editingCategory) {
        ...
        response = await axios.put(`blog-categories/${editingCategory._id}`, requestData, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        toast.success('Category updated successfully!');
      } else {
        // Create new category
        response = await axios.post('blog-categories', requestData, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      ...
      await axios.patch(`blog-categories/${id}/toggle`, {}, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
    ...
    <div className="container mx-auto p-6">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">Manage blog categories for organizing posts</p>
        </div>
        <button
